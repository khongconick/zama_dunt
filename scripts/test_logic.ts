/*
 Script kiểm thử logic ZamaFheDunt_Simple trên Sepolia
 Chạy:
...
// Địa chỉ contract (fallback). Ưu tiên lấy từ deployments/sepolia/ZamaFheDunt_Simple.json nếu có
...
function loadDeployedAddress(): string {
  try {
    const fp = path.join(__dirname, "../deployments/sepolia/ZamaFheDunt_Simple.json");
    if (fs.existsSync(fp)) {
...
async function main() {
  console.log("=== Test logic ZamaFheDunt_Simple ===");
