...
task("lucky-race:deploy", "Deploy ZamaFheDunt contract")
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    const { ethers } = hre;
    
    console.log("Deploying ZamaFheDunt contract...");
    const ZamaFheDunt = await ethers.getContractFactory("ZamaFheDunt");
    const luckyRaceFHE = await ZamaFheDunt.deploy();
    await luckyRaceFHE.waitForDeployment();
    
    const address = await luckyRaceFHE.getAddress();
    console.log("ZamaFheDunt deployed to:", address);
...
task("lucky-race:add-pool", "Add a new pool reward")
  ...
  .setAction(async (taskArgs, hre: HardhatRuntimeEnvironment) => {
    ...
    const ZamaFheDunt = await ethers.getContractFactory("ZamaFheDunt");
    const luckyRaceFHE = ZamaFheDunt.attach(taskArgs.contract);
